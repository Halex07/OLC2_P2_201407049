...
void generate_expression(CodeGenerator *gen, NodoBase *expr, int target_reg)
{
  ...
  else if (strcmp(expr->nombre, "Float") == 0)
  {
    generate_parseFloat(gen, expr);
    return;
  }
}
...
void generate_operation(CodeGenerator *gen, NodoBase *op, int target_reg)
{
  ...
  {
    // Bitwise XOR
    fprintf(gen->output_file, "    eor x%d, x%d, x%d\n", target_reg, reg_izq, reg_der);
  }
  else if (strcmp(o->op, "<<") == 0)
  {
    // Shift left (logical)
    fprintf(gen->output_file, "    lsl x%d, x%d, x%d\n", target_reg, reg_izq, reg_der);
  }
  else if (strcmp(o->op, ">>") == 0)
  {
    // Shift right (arithmetic)
    fprintf(gen->output_file, "    asr x%d, x%d, x%d\n", target_reg, reg_izq, reg_der);
  }
  else if (strcmp(o->op, "!=") == 0)
  {
    // Diferente: si x != y → 1, si no → 0
    fprintf(gen->output_file, "    cmp x%d, x%d\n", reg_izq, reg_der);
    fprintf(gen->output_file, "    cset x%d, ne\n", target_reg); // ne = not equal
  }
  else if (strcmp(o->op, "equals") == 0)
  {
    fprintf(gen->output_file, "    mov x0, x%d\n", reg_izq);
    fprintf(gen->output_file, "    mov x1, x%d\n", reg_der);
    fprintf(gen->output_file, "    bl stringEquals\n");
    fprintf(gen->output_file, "    mov x%d, x0\n", target_reg); // resultado (1 o 0)
  }
  else if (strcmp(o->op, "!") == 0)
  {
...
void generate_parseFloat(CodeGenerator *gen, NodoBase *float_expr)
{
    Float *p = (Float *)float_expr;

    fprintf(gen->output_file, "    # Float\n");

    // Evaluar la expresión que produce la cadena
    int reg_expr = allocate_register(gen->reg_manager);
    generate_expression(gen, p->expr, reg_expr);

    // Pasar string en x0
    fprintf(gen->output_file, "    mov x0, x%d\n", reg_expr);

    // Llamar a parseFloatFunc
    fprintf(gen->output_file, "    bl parseFloatFunc\n");

    // Liberar registro de la cadena
    free_register(gen->reg_manager, reg_expr);

    // Guardar el resultado float en un registro
    int target_reg = allocate_register(gen->reg_manager);
    fprintf(gen->output_file, "    fmov s%d, s0\n", target_reg); // resultado en sN
}
