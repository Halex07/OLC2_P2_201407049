...
void write_header(CodeGenerator *gen)
{
    ...
    fprintf(gen->output_file, "stringEquals:\n");
    fprintf(gen->output_file, "    stp x29, x30, [sp, #-16]!\n");
    fprintf(gen->output_file, "    mov x29, sp\n");
    fprintf(gen->output_file, "compare_loop:\n");
    fprintf(gen->output_file, "    ldrb w2, [x0], #1\n");
    fprintf(gen->output_file, "    ldrb w3, [x1], #1\n");
    fprintf(gen->output_file, "    cmp w2, w3\n");
    fprintf(gen->output_file, "    b.ne not_equal\n");
    fprintf(gen->output_file, "    cbz w2, equal\n");
    fprintf(gen->output_file, "    b compare_loop\n");
    fprintf(gen->output_file, "equal:\n");
    fprintf(gen->output_file, "    mov x0, #1\n");
    fprintf(gen->output_file, "    b end_compare\n");
    fprintf(gen->output_file, "not_equal:\n");
    fprintf(gen->output_file, "    mov x0, #0\n");
    fprintf(gen->output_file, "end_compare:\n");
    fprintf(gen->output_file, "    ldp x29, x30, [sp], #16\n");
    fprintf(gen->output_file, "    ret\n\n");

    // === Float.parseFloat ===
    fprintf(gen->output_file, "parseFloatFunc:\n");
    fprintf(gen->output_file, "    stp x29, x30, [sp, #-16]! \n");
    fprintf(gen->output_file, "    mov x29, sp\n");
    fprintf(gen->output_file, "    mov x1, xzr\n");
    fprintf(gen->output_file, "    bl printf\n"); // el resultado queda en d0
    fprintf(gen->output_file, "    fcvt s0, d0\n");
    fprintf(gen->output_file, "    ldp x29, x30, [sp], #16\n");
    fprintf(gen->output_file, "    ret\n\n");
}
